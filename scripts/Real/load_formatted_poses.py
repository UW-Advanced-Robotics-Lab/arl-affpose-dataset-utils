...
def main():
    ...
    for image_idx, image_addr in enumerate(img_files):
        ...
        mask_aff_label = np.ma.getmaskarray(np.ma.masked_not_equal(aff_label, 0)).astype(np.uint8)
        cv2_aff_mask_img = np.repeat(mask_aff_label, 3).reshape(mask_aff_label.shape[0], mask_aff_label.shape[1], -1) * cv2_aff_mask_img
        ...
        # cv2.imshow('aff_mask', cv2.cvtColor(cv2_aff_mask_img, cv2.COLOR_BGR2RGB))
